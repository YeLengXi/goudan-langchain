...
function parseArgs(args) {
  const config = { ...DEFAULT_CONFIG }; // Add support for custom indentation (2 spaces, 4 spaces, Tab)
  config.indentSize = args.includes('--indent=2') ? 2 : args.includes('--indent=4') ? 4 : args.includes('--tab') ? 'tab' : 2;
  const files = [];
  ...
  return { config, files }; // Add support for custom indentation (2 spaces, 4 spaces, Tab)
}
...
Examples:
  ...
  node code-formatter-v3.cjs -i 4 -o output.js input.js
  node code-formatter-v3.cjs -I input.js  (modify in place)');
}
